...
LOG_RETENTION_MAX="${LOG_RETENTION_MAX:-15}"
...
docker run -d \
  ...
  -e LOG_RETENTION_MAX="${LOG_RETENTION_MAX}" \
...
echo "调度: 每周日 00:00 执行（部署时不立即训练），日志保留: ${LOG_RETENTION_MAX} 个文件"
echo "首训窗口: ${WINDOW_DAYS} 天"
