...
from database.mongo.usage_table import add_machine_time, add_machine_time_to_old, add_machine_time_update
...
def machine_time_retest_old(imei,startTime, endTime):
    # 获取电流并解压缩
    dataList = get_current_by_time(imei, startTime, endTime + 1439 * 60 * 1000)
    modified_list = []
    if dataList:
        for dt in dataList:
...
def machine_time_retest_new(imei):
    ...
    if dt:
        ...
        # print(modified_list)
        # times = [item['time'] for item in modified_list]
...
def add_machine_time_new(imei,modified_list,deviceType):
    # add_machine_time_update(imei,modified_list,deviceType,1)
