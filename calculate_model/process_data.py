...
# 新修改预处理，生成训练数据----------------------------------------------------------------
def sequence_processing_new(data,feature_list,U_close,threshold):
    # 除去-1的序列
    padded_seqs = []
    # -1序列的位置
    labels_neg_and_closed = []
    ...
    for i,seq in enumerate(data):
        ...
        if std_dev > threshold:
            wave_seqs.append((i,3))  # 如果波动大，则添加到波动序列列表
            continue  # 跳过后续的处理步骤
        # 均值小于0的
        if mean_val < 0:
            labels_neg_and_closed.append((i,0))
...
if __name__ == "__main__":
    arr = [2,2,2,2,2,3,2,3,3,3,3,2,2,2,2,2,0,3,2,3,3,3,3,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,2,3,2,3,3,3,3,2,2,2,2,]
    res = compress_and_modify_states(arr,30,4)
    print(res)
